			...
			while (p != null) {
				...
				if (p != null) {
					...
					if (p.getWeight() + preweight > ve[temp]) {
						...
						System.out.println("ve[" + temp + "]:" + ve[temp]);
					}
			...
			if (begin != null) {
				...
				while (begin.getNext() != null) {
					...
					l[t++] = vl[((Integer) begin.getNext().getData())
							.intValue()] - begin.getNext().getWeight();
					begin = begin.getNext();
				}
...
//									.getElement(k)] - ver.getNext().getWeight();
//						}
//
//					}
//					ver = ver.getNext();
